...
if (!dir.exists("3_data_analysis/5-pathway_analysis/GSEA/wt_ctrl")) {
  dir.create(
    "3_data_analysis/5-pathway_analysis/GSEA/wt_ctrl",
    recursive = TRUE,
    showWarnings = FALSE
  )
}
...
setwd("3_data_analysis/5-pathway_analysis/GSEA/wt_ctrl")
...
transcriptome_data@variable_info$fc_wt_ctrl[transcriptome_data@variable_info$fc_wt_ctrl > 300]

transcriptome_data %>%
  activate_mass_dataset(what = "variable_info") %>%
  dplyr::filter(fc_wt_ctrl > 300) %>%
  extract_expression_data() %>%
  view()

transcriptome_data %>%
  activate_mass_dataset(what = "variable_info") %>%
  dplyr::filter(fc_wt_ctrl == 0) %>%
  extract_expression_data() %>%
  view()
...
gene_list <- setNames(data_all$fc_wt_ctrl, data_all$ENTREZID)
...
write.csv(go_results, file = c('GO/GSEA_GO_wt_ctrl.csv'))

go_ids <- go_results@result$ID
for (go_id in go_ids) {
  plot <- gseaNb(object = go_results, geneSetID = go_id)
  png_filename <- paste0("GO/GSEA_", go_id, "_wt_ctrl.png")
  pdf_filename <- paste0("GO/GSEA_", go_id, "_wt_ctrl.pdf")
  ggsave(plot,
...
write.csv(kegg_results, file = c('KEGG/GSEA_KEGG_wt_ctrl.csv'))


kegg_ids <- kegg_results@result$ID
for (kegg_id in kegg_ids) {
  plot <- gseaNb(object = kegg_results, geneSetID = kegg_id)
  png_filename <- paste0("KEGG/GSEA_", kegg_id, "_wt_ctrl.png")
  pdf_filename <- paste0("KEGG/GSEA_", kegg_id, "_wt_ctrl.pdf")
  ggsave(plot,
         filename = png_filename,
         width = 5,
         height = 4)
...
write.csv(rec_results, file = c('REACTOME/GSEA_Reactome_wt_ctrl.csv'))

rec_ids <- rec_results@result$ID
for (rec_id in rec_ids) {
  plot <- gseaNb(object = rec_results, geneSetID = rec_id)
  png_filename <- paste0("REACTOME/GSEA_", rec_id, "_wt_ctrl.png")
  pdf_filename <- paste0("REACTOME/GSEA_", rec_id, "_wt_ctrl.pdf")
  ggsave(plot,
         filename = png_filename,
         width = 5,
         height = 4)
