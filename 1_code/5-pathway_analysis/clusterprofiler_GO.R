...
##保存GO富集结果：
save(GO_MF_diff,GO_CC_diff,GO_BP_diff,GO_all_diff,file = c("GO_diff_BQ.5.2.Rdata"))
write.csv(GO_all_result, file = c('GO_all_diff_BA.5.2.csv'))
...
#CC：
pp2 <- GO_dot("CC2") + scale_color_distiller(palette = "YlGnBu",direction = 1)
pp2

#BP：
pp3<- GO_dot("BP2") + scale_color_distiller(palette = "YlOrBr",direction = 1)
pp3

ppp2 <- GO_dot("all")+scale_color_distiller(palette = "blues",direction = 1)
ppp2


ggsave(ppp2,
       filename = "GO_wt_ba52.png",
       width = 7,
       height = 6)

ggsave(ppp2,
       filename = "GO_wt_ba52.pdf",
       width = 7,
       height = 6)
