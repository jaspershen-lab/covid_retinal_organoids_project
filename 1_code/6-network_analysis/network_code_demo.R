...
load("3_data_analysis/data_20200511/gut_microbiome/variable_info")
gutmicrobiome_variable_info <- variable_info

setwd("3_data_analysis/data_20200511/environment_gutmicrobiome/")

load("gutmicrobiome_expression_data")
load("environment_expression_data")
load("gutmicrobiome_sample_info")
load("environment_sample_info")

load("environment_variable_info")

gutmicrobiome_sample_info$sample_id == colnames(gutmicrobiome_expression_data)
...
##correct fiber for gutmicrobime
gutmicrobiome_expression_data1 <-
  purrr::map(
    as.data.frame(t(gutmicrobiome_expression_data)),
    .f = function(x) {
      temp_data <-
...
temp_data <-
  apply(gutmicrobiome_expression_data1, 1, function(x) {
    (x - mean(x)) / sd(x)
  }) %>%
  t()

colnames(temp_data) <-
  colnames(gutmicrobiome_expression_data1) <-
  colnames(gutmicrobiome_expression_data)

##heatmap of gutmicrobiome
library(circlize)
...
ggsave(plot,
       filename = "gutmicrobiome_plot/gut_microbiome_heatmap.pdf",
       width = 7,
       height = 7)
...
####calculate correlation between gutmicrobiome and environment
cor_value <-
  cor(x = t(as.matrix(environment_expression_data)),
      y = t(as.matrix(gutmicrobiome_expression_data1)),
      method = "spearman")
...
p_value <-
  purrr::map(
    ...
    .f = function(x) {
      value1 <- as.numeric(environment_expression_data[x[1],])
      value2 <- as.numeric(gutmicrobiome_expression_data[x[2],])
...
setwd(r4projects::get_project_wd())
setwd("3_data_analysis/data_20200511/environment_gutmicrobiome/environment_gutmicrobiome_plot")
...
#   temp_data <-
#     data.frame(
#       date = as.character(gutmicrobiome_sample_info$CollectionDate),
#       environment = as.numeric(environment_expression_data[cor_value1$from[idx],]),
#       gutmicrobiome = as.numeric(gutmicrobiome_expression_data[cor_value1$to[idx],]),
#       stringsAsFactors = FALSE
#     )
#   plot <-
#     temp_data %>%
#     ggplot(aes(environment, gutmicrobiome)) +
#     geom_point() +
#     geom_smooth(method = "lm", color = "skyblue") +
#     ggrepel::geom_label_repel(aes(x = environment, gutmicrobiome, label = date)) +
#     labs(
#       x = paste("Exposome (Biological): ", cor_value1$from[idx], sep = ""),
#       y = paste("gutmicrobiome: " , cor_value1$to[idx]),
#       sep = ""
#     ) +
#     theme_bw() +
#     theme(
...
###correlation network for environment biological and gutmicrobiome
...
###network for all the environment biological and gutmicrobiome
edge_data <-
  cor_value1 %>%
...
ggsave(
  plot1,
  filename = "environment_gutmicrobiome_correlation_network.pdf",
  width = 8.5,
  height = 7,
  bg = "transparent"
)
...
ggsave(
  plot1,
  filename = "environment_gutmicrobiome_correlation_network.png",
  width = 8.5,
  height = 7,
  bg = "transparent"
)
