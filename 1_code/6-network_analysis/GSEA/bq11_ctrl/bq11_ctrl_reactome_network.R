...
output_dir <- "3_data_analysis/6-network_analysis/GSEA/bq11_ctrl/bq11_ctrl_reactome"
...
edge_data <- reactome_results %>%
  ...
  mutate(
    to = Description,
    Correlation = reactome_results$NES[match(Description, reactome_results$Description)],
    fdr = -log10(reactome_results$pvalue[match(Description, reactome_results$Description)])
  )
...
pathway_nodes <- data.frame(
  ...
  Class = "Pathway",
  size = -log10(reactome_results$pvalue[match(unique(edge_data$to), reactome_results$Description)]),
  color = reactome_results$NES[match(unique(edge_data$to), reactome_results$Description)],
