...
#####
if (!dir.exists("3_data_analysis/6-network_analysis/GSEA/bq11_ctrl/bq11_ctrl_kegg")) {
  dir.create(
    "3_data_analysis/6-network_analysis/GSEA/bq11_ctrl/bq11_ctrl_kegg",
    recursive = TRUE,
    showWarnings = FALSE
  )
}
...
setwd("3_data_analysis/6-network_analysis/GSEA/bq11_ctrl/bq11_ctrl_kegg")
...
# edge_data
  edge_data <- kegg_results %>%
    ...
    mutate(
      to = ID,
      Correlation = kegg_results$NES[match(ID, kegg_results$ID)],
      fdr = -log10(kegg_results$pvalue[match(ID, kegg_results$ID)])
    )
...
pathway_nodes <- data.frame(
  ...
  Class = "Pathway",
  size = -log10(kegg_results$pvalue[match(unique(edge_data$to), kegg_results$ID)]),
  color = kegg_results$NES[match(unique(edge_data$to), kegg_results$ID)],
