...
#####
if (!dir.exists("3_data_analysis/6-network_analysis/GSEA/bq11_ctrl/bq11_ctrl_reactome")) {
  dir.create(
    "3_data_analysis/6-network_analysis/GSEA/bq11_ctrl/bq11_ctrl_reactome",
    recursive = TRUE,
    showWarnings = FALSE
  )
}
...
setwd("3_data_analysis/6-network_analysis/GSEA/bq11_ctrl/bq11_ctrl_reactome")

#####reactome
# Top 10 pathways
top_pathways <- reactome_results%>%
  as.data.frame() %>%
...
# edge_data
  edge_data <- reactome_results %>%
    ...
    mutate(
      to = ID,
      Correlation = reactome_results$NES[match(ID, reactome_results$ID)],
      fdr = -log10(reactome_results$pvalue[match(ID, reactome_results$ID)])
    )
...
data_all <-
  ...
  dplyr::arrange(desc(fc_bq11_ctrl))
...
gene_nodes <- tibble(
  ...
  left_join(
    tibble(
      ...
      size = abs(log2(data_all$fc_bq11_ctrl)),
...
pathway_nodes <- data.frame(
  ...
  Class = "Pathway",
  size = -log10(reactome_results$pvalue[match(unique(edge_data$to), reactome_results$ID)]),
  color = reactome_results$NES[match(unique(edge_data$to), reactome_results$ID)],
...
ggsave(
  plot,
  filename = "top10_NES_pathway_network_bq11_ctrl_reactome.pdf",
  width = 8.5,
  height = 7,
  bg = "transparent",
  dpi = 300
...
ggsave(
  plot,
  filename = "top10_NES_pathway_network_bq11_ctrl_reactome.png",
  width = 8.5,
  height = 7,
  bg = "transparent",
  dpi = 300
