...
data <- transcriptome_data@variable_info[, c("SYMBOL", "fc_bq11_ctrl", "p_value_adjust_bq11_ctrl")]
go_results <- read_csv("3_data_analysis/5-pathway_analysis/GSEA/bq11_ctrl/GO/GSEA_go_bq11_ctrl.csv")
kegg_results <- read_csv("3_data_analysis/5-pathway_analysis/GSEA/bq11_ctrl/KEGG/GSEA_KEGG_bq11_ctrl.csv")
reactome_results <- read_csv("3_data_analysis/5-pathway_analysis/GSEA/bq11_ctrl/REACTOME/GSEA_reactome_bq11_ctrl.csv")

# Create output directory
output_dir <- "3_data_analysis/6-network_analysis/GSEA/bq11_ctrl"
...
gene_nodes <- tibble(
  ...
  left_join(
    tibble(
      ...
      size = -log10(data_all$p_value_adjust_bq11_ctrl),
      color = log2(data_all$fc_bq11_ctrl)
    ),
    by = "node"
  ) %>%
...
filtered_edges <- edge_data %>%
  ...
  group_modify(~{
    if(first(.x$pathway_nes) > 0) {
      arrange(.x, desc(fc_bq11_ctrl))  # NES正，选择fold change最大的基因
    } else {
      arrange(.x, fc_bq11_ctrl)        # NES负，选择fold change最小的基因
    }
  }) %>%
  slice_head(n = 3) %>%
  ungroup()
...
plot <- ggraph(temp_data, 
  ...
  alpha = ifelse(V(temp_data)$Class == "Pathway", 1, 0.5), 
  check_overlap = TRUE
  ) +
...
ggsave(
  plot,
  filename = "top10_NES_pathway_network_bq11_ctrl_all_top10.pdf",
  width = 10,
  height = 5,
  bg = "transparent",
  dpi = 300
...
ggsave(
  plot,
  filename = "top10_NES_pathway_network_bq11_ctrl_all_top10.png",
  width = 10,
  height = 5,
  bg = "transparent",
  dpi = 300
...
ggsave(
  barplot,
  filename = "top10_NES_pathway_network_bq11_ctrl_all_top10_barplot.pdf",
  width = 8.5,
  height = 7,
  bg = "transparent",
  dpi = 300
...
ggsave(
  barplot,
  filename = "top10_NES_pathway_network_bq11_ctrl_all_top10_barplot.png",
  width = 8.5,
  height = 7,
  bg = "transparent",
  dpi = 300
