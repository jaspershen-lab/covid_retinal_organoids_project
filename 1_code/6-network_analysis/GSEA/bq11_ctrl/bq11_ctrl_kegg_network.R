...
output_dir <- "3_data_analysis/6-network_analysis/GSEA/bq11_ctrl/bq11_ctrl_kegg"
...
edge_data <- kegg_results %>%
  ...
  mutate(
    to = Description,
    Correlation = kegg_results$NES[match(Description, kegg_results$Description)],
    fdr = -log10(kegg_results$pvalue[match(Description, kegg_results$Description)])
  )
...
pathway_nodes <- data.frame(
  ...
  Class = "Pathway",
  size = -log10(kegg_results$pvalue[match(unique(edge_data$to), kegg_results$Description)]),
  color = kegg_results$NES[match(unique(edge_data$to), kegg_results$Description)],
...
ggsave(
  plot,
  filename = "top10_NES_pathway_network_bq11_ctrl_kegg.pdf",
  width = 8.5,
  height = 7,
  bg = "transparent",
  dpi = 300
...
ggsave(
  plot,
  filename = "top10_NES_pathway_network_bq11_ctrl_kegg.png",
  width = 8.5,
  height = 7,
  bg = "transparent",
  dpi = 300
